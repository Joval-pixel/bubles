...
async function loadAndRender(){
  ...
  }catch(e){ console.error(e); setStatus("Erro ao carregar"); }
}

function todayISO(){ const d=new Date(),o=d.getTimezoneOffset();return new Date(d-o*60000).toISOString().slice(0,10) }
...
// Popularidade
const POP_TEAMS={"Flamengo":1,"Palmeiras":1,"Corinthians":0.95,"São Paulo":0.95,"Santos":0.9,"Vasco":0.9,"Grêmio":0.88,"Internacional":0.88,"Atlético Mineiro":0.9,"Cruzeiro":0.88,"Fluminense":0.9,
"Real Madrid":1,"Barcelona":1,"Atlético Madrid":0.92,"Manchester City":1,"Manchester United":1,"Liverpool":1,"Chelsea":0.95,"Arsenal":0.98,"Tottenham":0.9,"Bayern Munich":1,"Borussia Dortmund":0.95,"Paris Saint-Germain":1,"Marseille":0.85,"Juventus":0.98,"Inter":0.98,"Milan":0.98,"Benfica":0.9,"Porto":0.9,"Sporting CP":0.88};
const POP_LEAGUES={"Serie A":0.9,"Brasileirão":0.9,"Brazil":0.9,"Premier League":1,"England":1,"La Liga":0.95,"Spain":0.95,"Bundesliga":0.93,"Germany":0.93,"Ligue 1":0.9,"France":0.9,"Serie A (ITA)":0.94,"Italy":0.94,"Primeira Liga":0.86,"Portugal":0.86,"CONMEBOL":0.9};
...
function renderBubbles(items){
  ...
  const scores=items.map(i=>i.score), sMin=Math.min(...scores), sMax=Math.max(...scores);
...
function place(list, size, W, H, pad){
  for(let t=0;t<700;t++){
    ...
    if(!list.some(it=>Math.hypot((left+size/2)-(it.left+it.size/2),(top+size/2)-(it.top+it.size/2)) < (size/2+it.size/2+pad))) return {left,top};
  }
  return {left:pad,top:pad};
}
function colorForProb(p){ const t=Math.max(0,Math.min(1,p)); let r,g,b; if(t<=.5){const k=t/.5;r=lerp(231,241,k);g=lerp(76,196,k);b=lerp(60,15,k);} else {const k=(t-.5)/.5;r=lerp(241,46,k);g=lerp(196,204,k);b=lerp(15,113,k);} return `rgb(${r|0},${g|0},${b|0})` }
function lerp(a,b,t){return a+(b-a)*t} function randInt(a,b){return Math.floor(Math.random()*(b-a+1))+a}
...
function renderList(items){
  ...
  for(const m of items){
    ...
    li.textContent=`[${hhmm}] ${m.label} — Palpite: ${m.bestLabel} (${Math.round(m.bestProb*100)}%)`;
    listEl.appendChild(li);
  }
}

/* Modal com análise (últimos 10 jogos + H2H) */
