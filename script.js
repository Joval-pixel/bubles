/*************************************************
 * BUBLES - ENGINE DE BOLHAS ESTILO CRYPTOBUBBLES
...
const CONFIG = {
    ...
    COLORS: {
        GREEN_POSITIVE: { light: '#00ff88', dark: '#00cc66', glow: 'rgba(0, 255, 136, 0.6)' },
        RED_NEGATIVE: { light: '#ff4757', dark: '#ff3742', glow: 'rgba(255, 71, 87, 0.6)' },
        BLUE_NEUTRAL: { light: '#3742fa', dark: '#2f3542', glow: 'rgba(55, 66, 250, 0.6)' },
        GOLD_IMPORTANT: { light: '#ffa502', dark: '#ff6348', glow: 'rgba(255, 165, 2, 0.6)' },
        PURPLE_SPECIAL: { light: '#a55eea', dark: '#8854d0', glow: 'rgba(165, 94, 234, 0.6)' }
    }
};

// ===== UTILITÁRIOS =====
const $ = selector => document.querySelector(selector);
const formatTime = (date) => new Date(date).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', hour12: false });
...
class MatchDataGenerator {
    static leagues = {
        brasileirao: {
            ...
            importance: 0.8,
            teams: [
                { name: 'Flamengo', shield: '🔴' }, { name: 'Palmeiras', shield: '🟢' },
                { name: 'Corinthians', shield: '⚫' }, { name: 'São Paulo', shield: '🔴' },
                { name: 'Santos', shield: '⚪' }, { name: 'Vasco', shield: '⚫' },
                { name: 'Botafogo', shield: '⚫' }, { name: 'Fluminense', shield: '🟢' },
                { name: 'Internacional', shield: '🔴' }, { name: 'Grêmio', shield: '🔵' }
            ]
        },
        premier: {
            name: 'Premier League',
            importance: 0.95,
            teams: [
                { name: 'Manchester City', shield: '🔵' }, { name: 'Liverpool', shield: '🔴' },
                { name: 'Arsenal', shield: '🔴' }, { name: 'Chelsea', shield: '🔵' },
                { name: 'Manchester United', shield: '🔴' }, { name: 'Tottenham', shield: '⚪' },
                { name: 'Newcastle', shield: '⚫' }, { name: 'Brighton', shield: '🔵' }
            ]
        },
        laliga: {
            name: 'La Liga',
            importance: 0.9,
            teams: [
                { name: 'Real Madrid', shield: '⚪' }, { name: 'Barcelona', shield: '🔵' },
                { name: 'Atletico Madrid', shield: '🔴' }, { name: 'Sevilla', shield: '🔴' },
                { name: 'Valencia', shield: '🟠' }, { name: 'Real Betis', shield: '🟢' }
            ]
        },
        champions: {
            ...
            importance: 1.0,
            teams: [
                { name: 'Real Madrid', shield: '⚪' }, { name: 'Manchester City', shield: '🔵' },
                { name: 'Bayern Munich', shield: '🔴' }, { name: 'PSG', shield: '🔵' }
            ]
        }
    };
    ...
    static generateMatchData(homeTeam, awayTeam, league, gameTime) {
        ...
        const winProbability = 0.3 + Math.random() * 0.6;
        ...
        return {
            ...
            homeTeam, awayTeam, league: league.name, gameTime: gameTime.toISOString(),
            status, score,
            odds: { home: homeOdd, draw: drawOdd, away: awayOdd, over25: overOdd, under25: underOdd },
            winProbability, importance: Math.min(importance, 1),
            confidence: 0.4 + Math.random() * 0.6,
            market: 'over_under',
            bias: (overProbImplied - (1 - overProbImplied)) * 2,
...
class CryptoBubble {
    constructor(matchData, canvasWidth, canvasHeight) {
        ...
        this.baseRadius = CONFIG.MIN_BUBBLE_SIZE + (matchData.importance * (CONFIG.MAX_BUBBLE_SIZE - CONFIG.MIN_BUBBLE_SIZE));
    ...
    getColorScheme() {
        ...
        if (status === 'LIVE') return CONFIG.COLORS.GOLD_IMPORTANT;
        if (importance > 0.9) return CONFIG.COLORS.PURPLE_SPECIAL;
        if (bias > 0.3) return CONFIG.COLORS.GREEN_POSITIVE;
        if (bias < -0.3) return CONFIG.COLORS.RED_NEGATIVE;
        return CONFIG.COLORS.BLUE_NEUTRAL;
    }
    ...
    draw(ctx) {
        ...
        const highlightGradient = ctx.createRadialGradient(
            ...
            this.x - currentRadius * 0.4, this.y - currentRadius * 0.4, currentRadius * 0.5
        );
...
class BubbleEngine {
    ...
    showMatchModal(matchData) {
        ...
        $('#home-shield').textContent = matchData.homeTeam.shield;
        $('#away-shield').textContent = matchData.awayTeam.shield;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    log('Bubles inicializado com sucesso!');
});
