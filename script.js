/************ BUBLES — script.js (layout otimizado) ************/
...
/* ========== 5) Gerador de posições (espiral + relaxamento com grid) ========== */
function generateBubblePositions(data) {
  ...
  const CX = width / 2;
  const CY = height / 2;
  const HOLE_R = Math.min(width, height) * 0.10; // “zona morta” central
  ...
  // grid espacial (acelera vizinhança)
  ...
  // Relaxamento por colisão
  const ITER = 12;        // 10–16 deixa bem solto (ajuste fino)
  const REPEL = 0.45;     // força de repulsão
  const DAMP = 0.85;      // atrito
  ...
  for (let k = 0; k < ITER; k++) {
    for (const p of positions) {
      ...
      for (const q of near) {
        ...
        const minDist = p.radius + q.radius + 4; // espaço entre bordas
        if (dist < minDist) {
...
/* ========== 6) Renderização das bolhas (SVG responsivo) ========== */
function renderBubbles() {
  ...
  positions.forEach(({ x, y, radius, stock }) => {
    ...
    // Fonte proporcional ao raio
    const fs1 = Math.max(10, Math.min(16, radius * 0.36)); // símbolo
    const fs2 = Math.max(9,  Math.min(14, radius * 0.30)); // variação %
...
/* ========== 7) UI: contador e tooltip simples ========== */
...
/* ========== 9) Inicialização segura e resize otimizado ========== */
document.addEventListener('DOMContentLoaded', () => {
  // Pequeno atraso para o layout calcular altura do SVG
  setTimeout(renderBubbles, 50);
});
